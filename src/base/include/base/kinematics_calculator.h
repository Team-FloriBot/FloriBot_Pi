...
private:
  ...
  // Params
  double wheel_sep_{0.44};          // track width [m]
  double wheel_rad_{0.105};         // wheel radius [m]
  double ticks_per_rev_{2048.0};    // effective ticks per wheel revolution
  bool unwrap_modulo_{true};        // handle wrap 0..N..0
  int modulo_ticks_{2048};          // wrap period for ticks (if <=0 -> round(ticks_per_rev_))
